...
def extract_document_id(query_text):
    ...
    """
    # egex looks for "document" followed by an optional space or underscore, and then digits.
    match = re.search(r'document[_\s]?(\d+)', query_text, re.IGNORECASE)
    if match:
        doc_number = match.group(1)
        # return the canonical filename expected in the metadata
...
def retrieve_relevant_documents(query_text, n_results=15):
    ...
    if document_id:
        ...
        # this 'where' clause is the metadata filter
